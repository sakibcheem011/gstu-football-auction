...
export default function AdminAuctionDashboard() {
  ...
  const fetchPlayers = async () => {
    if (!token) return;
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL }/auction/players`, {
  ...
  useEffect(() => {
    if (token) {
      fetchPlayers();
      fetchTeams();
      fetch(`${process.env.NEXT_PUBLIC_API_URL }/rules/config`).then(res => res.json()).then(data => {
        setConfig(data);
  ...
  const startAuction = async (playerId: string, timer: number) => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL }/auction/start`, {
      method: 'POST',
  ...
  const cancelAuction = async () => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL }/auction/cancel`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` }
    });
    if (res.ok) {
  ...
  const sellPlayer = async () => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL }/auction/sell`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` }
    });
    if (res.ok) {
  ...
  const markUnsold = async () => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL }/auction/unsold`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` }
    });
    if (res.ok) {
